...
合并包含 [`.github/workflows/deploy-pages.yml`](.github/workflows/deploy-pages.yml) 的 PR 后，workflow 会在 push 到 `master` 或手动触发时构建并部署。**首次部署前，维护者须一次性启用 Pages**（GitHub Actions 的 `GITHUB_TOKEN` 无法调用 `POST /repos/.../pages`，会返回 403；`administration: write` 也不是合法的 workflow 权限）。

在仓库根目录执行（将 `<owner>` 换为实际组织/用户名，上游为 `watercooling480`）：
...
然后到 **Actions → Deploy GitHub Pages → Run workflow**，或再 push 一次 `master`。若 `deploy-pages` 报 404，通常是尚未执行上述命令。
